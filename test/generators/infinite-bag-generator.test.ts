...
describe('Infinite Bag generator', () => {
  ...
  beforeEach(() => {
    bagGenerator = new InfiniteBagGenerator(
      [],
      Generator.cloneQueue(bag),
      new LCG(1)
    );
  });
  ...
  it('Can refill', () => {
    ...
    const containsPieces = bag.every((bagPiece) => {
      return bagGenerator
        .getPreview(nbPreviewPieces)
        .some((piece) => piece.type === bagPiece.type);
    });
  ...
  it('Can make a copy of itseld with a new rng', () => {
    ...
    var { rng, ...copyNoRNG } = copy as any; // eslint-disable-line
